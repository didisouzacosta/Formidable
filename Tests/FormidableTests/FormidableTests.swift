...
struct FormidableTests {
    ...
    @Test func ensureIsValidWhenAllFieldsAreValid() throws {
        let form = SignInForm(email: "", password: "")
        form.emailField.rules = [RequiredRule(TestError.valueIsRequired)]
        form.passwordField.rules = [RequiredRule(TestError.valueIsRequired)]
        
        #expect(form.isValid == false)
        #expect(form.errors.count == 2)
        #expect(throws: TestError.valueIsRequired) {
            try form.validate()
        }
